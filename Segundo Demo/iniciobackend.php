...
<head>
	...
	<body id="color">		
			<div id="paisajemiperfil">
				<h1 id="tituloperfil"> Últimos Post </h1>
			</div>
					<?php
					$todos = mysql_query("SELECT tipo_couchs.nombre_tipocouch, couchs.id_couch, couchs.titulo, couchs.provincia, couchs.ciudad, couchs.id_tipo_couch, couchs.megusta, couchs.nomegusta, couchs.capacidad, couchs.descripcion,couchs.fecha_desde,couchs.fecha_hasta, couchs.fotocouch, usuarios.nombre_usuario,usuarios.es_premium FROM couchs INNER JOIN tipo_couchs on couchs.id_tipo_couch=tipo_couchs.id_tipo_couch INNER JOIN usuarios on usuarios.mail=couchs.mail WHERE couchs.baja_couch=0 ORDER BY couchs.id_couch DESC");
								...
								if($premium){
									...
									echo '<div id=imagen><img src="'.$fotopremium.' "width=100% height=100% /></div>';
											...
											//echo '<div id=imagen><img src="'.$resultado["fotocouch"].'"width=100% height=100%/></div>';
												...
												echo'<div id=boton><input type="hidden" name="id" value="'.$resultado["id_couch"].'"></div>';
