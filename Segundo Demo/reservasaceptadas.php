...
<head>
	...
	<body id="color">		
			<div id="paisajemiperfil">
				<h1 id="tituloperfil" style="color: white"> Reservas Aceptadas </h1>
			</div>
				...
				if($admin ==0){
					?>	
						<div id="botonesperfil">
								<a id="modificarperfil" href="solicitudes.php" class="btn btn-success"> Volver a solicitudes </a>
						</div>
				<?php
				}else{ 
					?>
									...
									echo '<form  method="POST">';
									 echo '<div id=imagen><img src="'.$resultado["fotocouch"].'"width=100% height=100%/></div>';
												...
												echo "</div>";
                                                                                                ...
                                                                                                //Las fechas tal vez no deberían enviarse por hidden, lo hice para solucionar rapidamente errores de POST
                                                                                                
                                                                                                echo'<div id=fechadesde><input type="hidden" name="fechadesde" value="'.$fecha.'">';
                                                                                                echo'<div id=hasta><input type="hidden" name="hasta" value="'.$fecha1.'">';
												echo'<div id=idcouch><input type="hidden" name="idcouch" value="'.$resultado["id_couch"].'">';    
												echo'<div id=boton><input type="hidden" name="idreserva" value="'.$resultado["id_reserva"].'">';
                                                                                                    
                                                                                                ?>            

												<input type="submit" id=verDetalles  name="CancelarReserva" onclick=action="cancelarreservahost.php" value="Cancelar Reserva Aceptada">
