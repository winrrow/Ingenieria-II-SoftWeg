...
<head>
	...
	<body id="color">		
			...
			?>
				<?php
					$todos = mysql_query("SELECT * FROM couchs INNER JOIN tipo_couchs on couchs.id_tipo_couch=tipo_couchs.id_tipo_couch WHERE mail='$mail' AND couchs.baja_couch=0 ORDER BY couchs.id_couch DESC ");
									...
									echo '<form action="modificarcouch.php"  method="POST">';
									 echo '<div id=imagen><img src="'.$resultado["fotocouch"].'"width=100% height=100%/></div>';
												...
												echo'<div id=boton><input type="hidden" name="id" value="'.$resultado["id_couch"].'">
														<input type="submit" id=verDetalles  name="verDetalles" value="Ver detalles" onclick=action="couch.php">
											                        <input type="submit" id=editar  name="editar" value="editar">
														<input type="submit" id=eliminar name="eliminar" value="eliminar" onclick=action="eliminarcouch.php">
