...
if(isset($_POST['eliminar'])){
	...
	$id_eliminar=$_POST['id'];
 	$sql= mysql_query("SELECT * FROM tipo_couchs  WHERE id_tipo_couch='$id_eliminar'",$con);
		while($row = mysql_fetch_array($sql)){
			$contador=$row['contador_usado'];
			$habilitado=$row['habilitado'];
			$nombre=$row['nombre_tipocouch'];
		}
		if(($contador == '0')AND($habilitado == 'no')){
			mysql_query("DELETE FROM tipo_couchs WHERE id_tipo_couch='$id_eliminar'",$con);
			?>
				<script type="text/javascript">
			...
			<?php
			  ...
			  if($contador > '0'){
			  		?>
						<script type="text/javascript">
							alert("El tipo de couch esta asginado por suarios, no se puede eliminar por el momento");
							location.href = "agregartipocouch.php";
						</script>	
					<?php
			  }
